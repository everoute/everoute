...
func NewIntermediateCertificateAuthority(parentCert *x509.Certificate, parentKey crypto.Signer, config *CertConfig, expireTime time.Time) (*x509.Certificate, crypto.Signer, error) {
	key, err := NewPrivateKey(config.PublicKeyAlgorithm)
	if err != nil {
		return nil, nil, errors.Wrap(err, "unable to create private key while generating intermediate CA certificate")
	}

	cert, err := NewSignedCert(config, key, parentCert, parentKey, expireTime, true)
	if err != nil {
...
func NewCertAndKey(caCert *x509.Certificate, caKey crypto.Signer, config *CertConfig, expireTime time.Time) (*x509.Certificate, crypto.Signer, error) {
	...
	cert, err := NewSignedCert(config, key, caCert, caKey, expireTime, false)
	if err != nil {
...
func TryLoadCertAndKeyFromDiskWithPath(certPath, keyPath string) (*x509.Certificate, crypto.Signer, error) {
	certs, err := certutil.CertsFromFile(certPath)
	if err != nil {
		return nil, nil, errors.Wrapf(err, "couldn't load the certificate file %s", certPath)
	}
...
func NewSignedCert(cfg *CertConfig, key crypto.Signer, caCert *x509.Certificate, caKey crypto.Signer, expireTime time.Time, isCA bool) (*x509.Certificate, error) {

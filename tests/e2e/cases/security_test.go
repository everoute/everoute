...
var _ = Describe("SecurityPolicy", func() {
	...
	Context("environment with virtual machines provide public http service [Feature:TCP] [Feature:ICMP]", func() {
		var nginx, server01, server02, db01, db02, client *VM
		...
		BeforeEach(func() {
			...
			nginx = &VM{Name: "nginx", TCPPort: nginxPort, Labels: "component=nginx"}
			server01 = &VM{Name: "server01", TCPPort: serverPort, Labels: "component=webserver"}
			server02 = &VM{Name: "server02", TCPPort: serverPort, Labels: "component=webserver"}
			db01 = &VM{Name: "db01", TCPPort: dbPort, Labels: "component=database"}
			db02 = &VM{Name: "db02", TCPPort: dbPort, Labels: "component=database"}
			client = &VM{Name: "client"}
		...
		When("limits tcp packets between components", func() {
			...
			BeforeEach(func() {
				...
				allowConnectionWith(nginxPolicy, "TCP", nginxPort, "", 0) // allow all connection with nginx port
				allowConnectionWith(nginxPolicy, "TCP", 0, serverGroup.Name, serverPort)

				serverPolicy = newPolicy("server-policy", tier1, 50, serverGroup.Name)
				allowConnectionWith(serverPolicy, "TCP", serverPort, nginxGroup.Name, 0)
				allowConnectionWith(serverPolicy, "TCP", 0, dbGroup.Name, dbPort)

				dbPolicy = newPolicy("db-policy", tier1, 50, dbGroup.Name)
				allowConnectionWith(dbPolicy, "TCP", dbPort, serverGroup.Name, 0)
				allowConnectionWith(dbPolicy, "TCP", 0, dbGroup.Name, 0) // allow connection between database
			...
			It("should allow normal packets and limits illegal packets", func() {
				assertReachable([]*VM{nginx}, []*VM{db01, db02}, "TCP", false)
				assertReachable([]*VM{client}, []*VM{server01, server02, db01, db02}, "TCP", false)

				assertReachable([]*VM{client}, []*VM{nginx}, "TCP", true)
				assertReachable([]*VM{nginx}, []*VM{server01, server02}, "TCP", true)
				assertReachable([]*VM{server01, server02, db01, db02}, []*VM{db01, db02}, "TCP", true)
			})
			...
			When("add virtual machine into the database group", func() {
				...
				It("should allow normal packets and limits illegal packets for new member", func() {
					assertReachable([]*VM{nginx, client}, []*VM{db03}, "TCP", false)
					assertReachable([]*VM{server01, server02, db01, db02}, []*VM{db03}, "TCP", true)
				})
			})
			...
			When("remove virtual machine from the webserver group", func() {
				...
				It("should limits illegal packets for remove member", func() {
					assertReachable([]*VM{server02}, []*VM{server01, db01, db02}, "TCP", false)
				})
			})
		})
		...
		When("limits icmp packets between components", func() {
			...
			BeforeEach(func() {
				...
				allowConnectionWith(icmpAllowPolicy, "ICMP", 0, "", 0) // allow all icmp packets
			...
			It("should allow normal packets and limits illegal packets", func() {
				assertReachable([]*VM{client}, []*VM{server01, server02, db01, db02}, "ICMP", false)
				assertReachable([]*VM{client}, []*VM{nginx}, "ICMP", true)
			})
		})
	...
	Context("environment with virtual machines provide internal udp service [Feature:UDP] [Feature:IPBlocks]", func() {
		...
		BeforeEach(func() {
			...
			client01 = &VM{Name: "ntp-client01", ExpectCidr: productionCidr}
			client02 = &VM{Name: "ntp-client02", ExpectCidr: developmentCidr}
			ntp01 = &VM{Name: "ntp01-server", ExpectCidr: productionCidr, UDPPort: ntpPort, Labels: "component=ntp,env=production"}
			ntp02 = &VM{Name: "ntp02-server", ExpectCidr: developmentCidr, UDPPort: ntpPort, Labels: "component=ntp,env=development"}
		...
		When("limits udp packets by ipBlocks between server and client", func() {
			...
			BeforeEach(func() {
				...
				allowConnectionWith(ntpProductionPolicy, "UDP", ntpPort, productionCidr, 0)

				ntpDevelopmentPolicy = newPolicy("ntp-development-policy", tier1, 50, ntpDevelopment.Name)
				allowConnectionWith(ntpDevelopmentPolicy, "UDP", ntpPort, developmentCidr, 0)
			...
			It("should allow normal packets and limits illegal packets", func() {
				...
				assertReachable([]*VM{ntp01}, []*VM{ntp02}, "UDP", false)
				assertReachable([]*VM{ntp02}, []*VM{ntp01}, "UDP", false)
...
// allowConnectionWith set traffic allow between applied groups and peer.
// Peer is group peer or cidr peer, format like: group01 or 10.0.0.0/24.
func allowConnectionWith(policy *securityv1alpha1.SecurityPolicy, protocol string, localPort int, peer string, peerPort int) {
	var ingressRule, egressRule *securityv1alpha1.Rule
		...
		Ports: []securityv1alpha1.SecurityPolicyPort{
			{
				...
				PortRange: strconv.Itoa(localPort),
			},
		},
		From: policyPeer,
	}
	...
	egressRule = &securityv1alpha1.Rule{
		...
		Ports: []securityv1alpha1.SecurityPolicyPort{
			{
				Protocol:  securityv1alpha1.Protocol(protocol),
				PortRange: strconv.Itoa(peerPort),
			},
		},
		To: policyPeer,
	}
...
func assertReachable(sources []*VM, destinations []*VM, protocol string, expectReach bool) {
	Eventually(func() error {
		var errList []error

...
func New(towerFactory informer.SharedInformerFactory, crdFactory externalversions.SharedInformerFactory, crdClient clientset.Interface, resyncPeriod time.Duration) *controller {
...
func (c *controller) addLabel(new interface{}) {
	...
	_ = c.enqueueVMNicsByVMReference(true, label.VMs...)
}
...
func (c *controller) updateLabel(old interface{}, new interface{}) {
	...
	_ = c.enqueueVMNicsByVMReference(true, oldLabel.VMs...)
	_ = c.enqueueVMNicsByVMReference(true, newLabel.VMs...)
}
...
func (c *controller) enqueueVMNicsByVMReference(ignoreNotFound bool, references ...schema.ObjectReference) error {
	var errList []error

	for _, reference := range references {
		vm, exists, err := c.vmLister.GetByKey(reference.ID)
		if err != nil {
		...
		if !exists {
			if !ignoreNotFound {
				errList = append(errList, fmt.Errorf("vm %s not found", reference.ID))
			}
			continue
		}

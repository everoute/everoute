...
func testProcessAgentinfo(t *testing.T) {
	...
	t.Run("agentinfo-added", func(t *testing.T) {
		...
		r.addEndpoint(event.CreateEvent{
			Meta:   fakeEndpointB.GetObjectMeta(),
		...
		if !EqualEndpointStatus(ovsPortStatusC, endpointStatus) {
	...
	t.Run("agentinfo-updated", func(t *testing.T) {
		...
		if !EqualEndpointStatus(*newOvsPortStatusC, endpointStatusB) {
			t.Errorf("unmatch endpoint status, get %v, want %v", endpointStatusB, newOvsPortStatusC)
		}

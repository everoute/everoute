...
const (
	...
	VMNicDriverTun       = "tun"
	VMNicDriverVhostuser = "dpdkvhostuserclient"
	PodNicDriver         = "veth"
)
...
func NewAgentMonitor(opts *NewAgentMonitorOptions) *AgentMonitor {
	if opts.OVSDBMonitor.GetSyncQueue() == nil {
		klog.Fatal("OVSDBMonitor syncQueue is nil, AgentMonitor need it to notify ovsdb update")
	}
	return &AgentMonitor{
		...
		for _, port := range bridge.Ports {
			for _, iface := range port.Interfaces {
				...
				for ip, info := range iface.IPMap {
					...
					if !endpointIP.IP.IsGlobalUnicast() && !endpointIP.IP.IsLinkLocalUnicast() {

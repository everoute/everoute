...
func (m *AgentMetric) SetTRNicMount(bridge string, in, out, all bool) {
	m.trNicMount.WithLabelValues(bridge, "nic_in").Set(boolToFloat64(in))
	m.trNicMount.WithLabelValues(bridge, "nic_out").Set(boolToFloat64(out))
	m.trNicMount.WithLabelValues(bridge, "nic_all").Set(boolToFloat64(all))
}

func (m *AgentMetric) SetTRHealthy(bridge string, linkIn, linkOut uint8, dpi, all bool) {
	m.trNicStatus.WithLabelValues(bridge, "nic_in").Set(float64(linkIn))
	m.trNicStatus.WithLabelValues(bridge, "nic_out").Set(float64(linkOut))

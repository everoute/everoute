...
func excuteCommand(commandStr string) ([]byte, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("failed to excute cmd: %s, error: %v", commandStr, err)
	}
...
func getIfaceID(nic string) (string, error) {
	...
	out := strings.Trim(strings.ReplaceAll(string(res), "\n", ""), `"`)
	return out, nil
}
...
func (p *Port) checkTRNicInSvcChain(ifaceID string) bool {
	if p.brName != tr.SvcChainBridgeName {
		klog.Infof("Port %s mount bridge is %s, not svcchain bridge", p.name, p.brName)
		return false
	}
...
func getPortInfo(idOrName string) (*Port, error) {
	...
	if len(resStr) != 3 {
		klog.Errorf("Invalid port info: %s", string(res))
...
func UnmountTRNic(ovsbrName string, d types.NicDirect) error {
	...
	if !p.checkTRNicHasMount(nicCfg.IfaceID, policyBrName) {
...
func findTrafficRedirectNic(ovsbrName, ifaceID string, d types.NicDirect) (string, error) {
	...
	resStr := strings.TrimSuffix(string(resS), "\n")
	if resStr == "" {
	...
	if len(res) > 1 {
		klog.Errorf("find multi interface with ifaceID %s for ovs bridge %s direct %s, res: %s", ifaceID, ovsbrName, d, string(resS))

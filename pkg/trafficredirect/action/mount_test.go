...
var _ = Describe("mount unit test", func() {
	Describe("getPortInfo", func() {
		...
		var fakeCmdOutput []byte

		BeforeEach(func() {
			patches = gomonkey.NewPatches()

			// 默认 mock 成功返回 3 行结果
			fakeCmdOutput = []byte("uuid1\nport1\n[intf1]")

			patches.ApplyFunc(excuteCommand, func(cmd string) ([]byte, error) {
				if strings.Contains(cmd, "port-to-br") {
					return []byte("br-test"), nil
				}
		...
		It("returns nil if excuteCommand fails", func() {
			patches.ApplyFunc(excuteCommand, func(string) ([]byte, error) {
				return nil, fmt.Errorf("failed command")
			})
	...
	Describe("findTrafficRedirectNic", func() {
		...
		It("should return error when output format is invalid", func() {
			p := gomonkey.ApplyFuncReturn(excuteCommand, []byte("iface-id:test:9"), nil)
			defer p.Reset()

...
var (
	...
	ovsCtrlPortStart int = 40000
	datapathConfig       = Config{
		ManagedVDSMap: map[string]string{
			"ovsbr0": "ovsbr0",
		},
		OvsCtrlPortStart: &ovsCtrlPortStart,
	}
...
func TestLocalEndpoint(t *testing.T) {
	t.Run("Test add local endpoint", func(t *testing.T) {
		...
		roundInfo, _ := getRoundInfo(datapathManager.OvsdbDriverMap["ovsbr0"][LOCAL_BRIDGE_KEYWORD])
		if roundInfo.curRoundNum != 2 || roundInfo.previousRoundNum != 1 {
			t.Errorf("Failed to get correct roundinfo")
		}
	})
}

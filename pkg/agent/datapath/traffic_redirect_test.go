	...
	tests := []struct {
		...
		},
		{
			...
			ep: &Endpoint{
				...
				InterfaceName: "if3",
				InterfaceUUID: "uuid3",
				Extend:        &EndpointExtend{IfaceID: "nic-out"},
			},
			expectRemount: true,
			expectUpdate:  false,
			expectError:   false,
...
func TestUpdateTREndpoint(t *testing.T) {
	...
	t.Run("ifaceID empty", func(t *testing.T) {
		ep := &Endpoint{InterfaceName: "ep1", BridgeName: "br1-policy", Extend: &EndpointExtend{}}
		err := dm.UpdateTREndpoint(ep)
		assert.NoError(t, err)
		mb := dm.BridgeChainMap["vds1"][POLICY_BRIDGE_KEYWORD].(*MockBridge)
		assert.False(t, mb.UpdateCalled)
	...
	t.Run("bridge is svcChainBridge", func(t *testing.T) {
		ep := &Endpoint{InterfaceName: "ep1", BridgeName: trconst.SvcChainBridgeName, Extend: &EndpointExtend{IfaceID: "id"}}
		err := dm.UpdateTREndpoint(ep)
		assert.NoError(t, err)
		mb := dm.BridgeChainMap["vds1"][POLICY_BRIDGE_KEYWORD].(*MockBridge)
		assert.False(t, mb.UpdateCalled)
	...
	t.Run("bridge suffix no policy", func(t *testing.T) {
		ep := &Endpoint{InterfaceName: "ep1", BridgeName: "br1-noPolicy", Extend: &EndpointExtend{IfaceID: "id"}}
		err := dm.UpdateTREndpoint(ep)
		assert.NoError(t, err)
		mb := dm.BridgeChainMap["vds1"][POLICY_BRIDGE_KEYWORD].(*MockBridge)
		assert.False(t, mb.UpdateCalled)
	...
	t.Run("bridge managed and UpdateTREndpoint called", func(t *testing.T) {
		ep := &Endpoint{InterfaceName: "ep1", BridgeName: "br1-policy", Extend: &EndpointExtend{IfaceID: "id"}}
		mb := dm.BridgeChainMap["vds1"][POLICY_BRIDGE_KEYWORD].(*MockBridge)
		mb.UpdateCalled = false
		mb.receivedEp = nil
	...
	t.Run("bridge managed but localBridge not match", func(t *testing.T) {
		ep := &Endpoint{InterfaceName: "ep1", BridgeName: "br2-policy", Extend: &EndpointExtend{IfaceID: "id"}}
		mb := dm.BridgeChainMap["vds1"][POLICY_BRIDGE_KEYWORD].(*MockBridge)
		mb.UpdateCalled = false

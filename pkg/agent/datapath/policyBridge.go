...
func (p *PolicyBridge) setupCTCommitFlows(baseMatch ofctrl.FlowMatch, l4matches []L4FlowMatch, actions ...openflow.Action) error {
	for _, l4match := range l4matches {
		...
		commitAction, err := ofctrl.NewConntrackActionWithZoneField(
			true,
			false,
...
func (p *PolicyBridge) initCTCommitTableCommitFlow() error {
	...
	for _, m := range []FlowMatchInportActions{
		{Inport: fromLocalInport, Actions: moveSourceActionActions},
		{Inport: fromClsInport, Actions: moveTargetActionActions},
	} {
		baseMatch := ofctrl.FlowMatch{
			Priority:  MID_MATCH_FLOW_PRIORITY,
	...
	for _, m := range []FlowMatchInportActions{
		{Inport: fromLocalInport, Actions: moveTargetActionActions},
		{Inport: fromClsInport, Actions: moveSourceActionActions},
	} {
		baseMatch := ofctrl.FlowMatch{
			Priority:  MID_MATCH_FLOW_PRIORITY,
	...
	err := p.setupCTCommitFlows(baseMatch, protocols, moveReplyInportActions...)
	if err != nil {

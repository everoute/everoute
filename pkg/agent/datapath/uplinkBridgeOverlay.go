...
var (
	UBOOutputPortReg   string            = "nxm_nx_reg2"
	UBOOutputPortStart int               = 0
...
func (u *UplinkBridgeOverlay) initArpProxytable() error {
	...
	podProxyFlow, _ := u.arpProxyTable.NewFlow(ofctrl.FlowMatch{
		...
		ArpTpaMask: (*net.IP)(&u.datapathManager.Info.ClusterCIDR.Mask),
...
func (u *UplinkBridgeOverlay) initSetTunnelOutPortTable() error {
	...
	if err := flow.LoadField(UBOOutputPortReg, uint64(u.datapathManager.Info.GatewayOfPort), UBOOutputPortRange); err != nil {
...
func (u *UplinkBridgeOverlay) initPaddingL2table() error {
	...
	if err := toGwFlow.SetMacDa(fackMac); err != nil {
...
func (u *UplinkBridgeOverlay) initOutputTable() error {
	...
	inPortFlow, _ := u.outputTable.NewFlow(ofctrl.FlowMatch{
		InputPort: u.datapathManager.BridgeChainPortMap[u.name][UplinkToClsSuffix],
		Regs: []*ofctrl.NXRegister{
			{
				RegID: constants.OVSReg2,
				Data:  u.datapathManager.BridgeChainPortMap[u.name][UplinkToClsSuffix],
				Range: UBOOutputPortRange,
			},
		},
		Priority: HIGH_MATCH_FLOW_PRIORITY,

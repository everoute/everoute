...
var (
	LBOOutputPortReg   string            = "nxm_nx_reg2"
	LBOOutputPortStart int               = 0
...
func (l *LocalBridgeOverlay) initFromLocalTable() error {
	...
	if err := svcFlow.LoadField("nxm_nx_pkt_mark", uint64(SvcPktMarkValue), SvcPktMarkRange); err != nil {
	...
	if l.enableProxy {
		...
		if err := podFlow.SetConntrack(ctAction); err != nil {
			return fmt.Errorf("failed to setup from local table pod flow set conntrack action, err: %v", err)
		}
	}

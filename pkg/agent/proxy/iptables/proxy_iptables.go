...
func (k *kubeProxy) everouteOutput(ipt *iptables.IPTables) []string {
	...
	if exist, err = ipt.Exists("nat", "EVEROUTE-OUTPUT", ruleSpec...); err != nil {
		klog.Errorf("Check %s in nat EVEROUTE-OUTPUT error, err: %s", k.localGwName, err)
	}
	if !exist {
...
func (k *kubeProxy) forward(ipt *iptables.IPTables) {
	...
	if exist, err = ipt.Exists("filter", "FORWARD", "-i", k.localGwName,
		"-m", "conntrack", "--ctstate", "INVALID", "-j", "ACCEPT"); err != nil {
		klog.Errorf("Check filter FORWARD error, err: %s", err)
	}
	if !exist {
		if err = ipt.Insert("filter", "FORWARD", 1, "-i", k.localGwName,
			"-m", "conntrack", "--ctstate", "INVALID", "-j", "ACCEPT"); err != nil {
...
func (k *kubeProxy) prerouting(ipt *iptables.IPTables) {
	...
	if exist, err = ipt.Exists("raw", "PREROUTING", "-i", k.localGwName, "-j", "CT", "--zone", "65510"); err != nil {
		klog.Errorf("Check %s in raw PREROUTING error, err: %s", k.localGwName, err)
	}
	if !exist {

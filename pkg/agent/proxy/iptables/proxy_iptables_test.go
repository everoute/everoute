...
func TestGetRuleSpecByIPSet(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			exp:  "-p tcp -m set --match-set er-npsvc-tcp dst -j MARK --set-mark 0x10000000/0x10000000",
		},
		{
			name: "nodeport svc udp",
			arg:  constants.IPSetNameNPSvcUDP,
			exp:  "-p udp -m set --match-set er-npsvc-udp dst -j MARK --set-mark 0x10000000/0x10000000",
		},
		{
			name: "lb svc",
			arg:  constants.IPSetNameLBSvc,
			exp:  "-m set --match-set er-lbsvc dst,dst -j MARK --set-mark 0x10000000/0x10000000",
		},
	}

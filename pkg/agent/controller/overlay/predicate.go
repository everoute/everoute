...
func endpointPredicate() predicate.Predicate {
	return predicate.Funcs{
		...
		UpdateFunc: func(e event.UpdateEvent) bool {
			new, newOk := e.ObjectNew.(*v1alpha1.Endpoint)
			old, oldOk := e.ObjectOld.(*v1alpha1.Endpoint)
			if !newOk || !oldOk {
				klog.Errorf("Endpoint update event transform to endpoint resource failed, event: %v", e)
				return false
			}

			if !endpointIPsEqual(new.Status.IPs, old.Status.IPs) {
				return true
			}
			if !endpointAgentsEqual(new.Status.Agents, old.Status.Agents) {
				return true
			}
			return false
		},
...
func nodePredicate(localNode string) predicate.Predicate {
	return predicate.Funcs{
		...
		UpdateFunc: func(e event.UpdateEvent) bool {
			...
			if utils.GetNodeInternalIP(*old) != utils.GetNodeInternalIP(*new) {
				return true
			}
			return false
		},

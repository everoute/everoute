...
type Reconcile struct {
	...
	// svcPortCache store each ServicePort resource related service namespacedName and service portName,
	// so when delete ServicePort resource, we can know which service port was deleted
...
// ReconcileServicePort receive servicePort from work queue
...
func (r *Reconcile) processServiceAdd(ctx context.Context, new *proxycache.BaseSvc) error {
	...
	if err := r.baseSvcCache.Add(new); err != nil {
		klog.Errorf("Failed to add %+v to baseSvcCache", new)
		return err
	}
...
func (r *Reconcile) updateServicePort(ctx context.Context, servicePort *everoutesvc.ServicePort) error {
	...
	_, exists, err := r.svcPortCache.GetByKey(svcPortKey)
	if err != nil {
	...
	return fmt.Errorf("updateServicePortForGroup err is %s, updateServicePortForBackend err is %s", err1, err2)
}
...
func (r *Reconcile) deleteBackendSvcPortRef(servicePort *everoutesvc.ServicePort) error {
	...
	for i := range oldBackends {
		...
		old := oldBackends[i].(*proxycache.Backend)
		exists := false
...
func (r *Reconcile) addBackendSvcPortRef(servicePort *everoutesvc.ServicePort) error {
	...
	dpNatBrs := r.DpMgr.GetNatBridges()
	svcPortRef := proxycache.GenServicePortRef(servicePort.Namespace, servicePort.Spec.PortName, servicePort.Spec.PortName)
	for i := range servicePort.Spec.Backends {
		...
		if !exists {
			...
			if oldObj == nil {
				klog.Errorf("The old backend is nil for servicePort %s backend %+v, update backend cache failed", svcPortRef, cur)
				continue
			}
			old := oldObj.(*proxycache.Backend)
...
func (r *Reconcile) deleteServicePortForBackend(svcPort *proxycache.SvcPort) error {
	...
	for i := range backends {
		...
		b := backends[i].(*proxycache.Backend)
